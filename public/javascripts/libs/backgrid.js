...
_.extend(CellFormatter.prototype, {
  ...
  /**
     Takes a raw value from a models and returns an optionally formatted string
  ...
  */
  ...
  /**
     ...
     appropriately typed value for persistence in the models.

     If the user input is invalid or unable to be converted to a raw value
     suitable for persistence in the models, toRaw must return `undefined`.
...
_.extend(PercentFormatter.prototype, {
  ...
  /**
     ...
     @cfg {number} [options.multiplier=1] The number used to multiply the models
     value for display.
  ...
  */
  ...
  /**
     ...
     and convert it back to a number for the models like NumberFormatter#toRaw,
...
/**
   ...
   If the type of a models value is not a string, it is expected that a subclass
...
var CellEditor = Backgrid.CellEditor = Backbone.View.extend({
  ...
  /**
     ...
     @param {Backbone.Model} options.models

     @throws {TypeError} If `formatter` is not a formatter instance, or when
     `models` or `column` are undefined.
  */
...
var InputCellEditor = Backgrid.InputCellEditor = CellEditor.extend({
  ...
  */
  ...
  /**
     ...
     in the editor to a raw value for saving into the models using the formatter.
     ...
     Triggers a Backbone `backgrid:edited` event from the models when successful,
...
/**
   ...
   table cell with the models value converted to a string using the
...
var Cell = Backgrid.Cell = Backbone.View.extend({
  ...
  */
  ...
  /**
     ...
     models's raw value for this cell's column.
  */
  render: function () {
    this.$el.empty();
    var model = this.model;
  ...
  /**
     ...
     This method triggers a Backbone `backgrid:edit` event from the models when
     ...
     `backgrid:editing` event is triggered from the models. The cell and the
     constructed cell instance are also sent as parameters in the event.

     When the models triggers a `backgrid:error` event, it means the editor is
     unable to convert the current user input to an apprpriate value for the
     models's column, and an `error` CSS class is added to the cell accordingly.
  */
...
var BooleanCellEditor = Backgrid.BooleanCellEditor = CellEditor.extend({
  ...
  /**
     Renders a checkbox and check it if the models value of this column is true,
     uncheck otherwise.
  */
  render: function () {
    var model = this.model;
  ...
  */
  ...
  /**
     Event handler. Save the value into the models if the event is `change` or
...
/**
   ...
   checkbox is checked if the models value is true, unchecked otherwise.
...
var SelectCellEditor = Backgrid.SelectCellEditor = CellEditor.extend({
  ...
  */
  ...
  /**
     Saves the value of the selected option to the models attribute.
  */
  save: function (e) {
    var model = this.model;
    var column = this.column;
  ...
  /**
     Triggers a `backgrid:edited` event from the models so the body can close
     this editor.
  */
  close: function (e) {
    var model = this.model;
...
/**
   ...
   During display mode, the default formatter will normalize the raw models value
   to an array of values whether the raw models value is a scalar or an
   ...
   conversion is performed when saving into the models. This behavior is not
...
var Column = Backgrid.Column = Backbone.Model.extend({
  ...
  /**
     ...
     @cfg {string} [defaults.name] The default name of the models attribute.
     ...
     formatter to use to convert between raw models values and user input.
     ...
     The function to use to extract a value from the models for comparison during
  ...
  */
  ...
  /**
     ...
     @param {string} attrs.name The models attribute this column is responsible
     for.
  ...
  /**
     Returns an appropriate value extraction function from a models for sorting.

     If the column models contains an attribute `sortValue`, if it is a string, a
     ...
     found from the column models's attributes, a default value extraction
...
var Columns = Backgrid.Columns = Backbone.Collection.extend({
  ...
  /**
     @property {Backgrid.Column} models
   */
  model: Column
});
...
/**
   Row is a simple container view that takes a models instance and a list of
   column metadata describing how each of the models's attribute is to be
...
var Row = Backgrid.Row = Backbone.View.extend({
  ...
  /**
     ...
     @param {Backbone.Model} options.models The models instance to render.

     @throws {TypeError} If options.columns or options.models is undefined.
  */
  initialize: function (options) {
  ...
  */
  ...
  /**
     Renders a row of cells for this row's models.
  */
  render: function () {
    this.$el.empty();
...
var Body = Backgrid.Body = Backbone.View.extend({
  ...
  */
  ...
  /**
     ...
     When called directly, it accepts a models or an array of models and an
     option hash just like
     [Backbone.Collection#add](http://backbonejs.org/#Collection-add) and
     delegates to it. Once the models is added, a new row is inserted into the
     body and automatically rendered.

     When called as a callback of an `add` event, splices a new row into the
     body and renders it.

     @param {Backbone.Model} model The models to render as a row.
  ...
  */
  ...
  /**
     ...
     delegates to it. Once the models is removed, a corresponding row is removed
     from the body.

     When called as a callback of a `remove` event, splices into the rows and
     removes the row responsible for rendering the models.

     @param {Backbone.Model} model The models to remove from the body.
     @param {Backbone.Collection} collection When called directly, this
     parameter is actually the options to
     [Backbone.Collection#remove](http://backbonejs.org/#Collection-remove).
     @param {Object} options When called directly, this must be null.
  ...
  */
  ...
  /**
     ...
     Triggers a `backgrid:next` event on the models with the indices of the row
     ...
     @param {Backbone.Model} model The originating models
     @param {Backgrid.Column} column The originating models column
  ...
  */
  moveToNextCell: function (model, column, command) {
    ...
    // return if models being edited in a different grid
    if (j === -1) return this;
...
/**
   ...
   By default, a Grid treats each models in a collection as a row, and each
   attribute in a models as a column. To render a grid you must provide a list of
...
var Grid = Backgrid.Grid = Backbone.View.extend({
  ...
  /**
     ...
     @param {Backbone.Collection} options.collection The collection of tabular models data to display.
